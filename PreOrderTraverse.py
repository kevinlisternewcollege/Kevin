...
def preorderTraverse (p):
    ...
    if tree[p][0] != -1:
        ...
        preorderTraverse(tree[p][0])  # note the use of the double indexing and the function recursively calling itself
    else:
        print(p,tree[p][1])
    if tree[p][2] != -1:
        print("loop 2","p=",p,tree[p][2])
        preorderTraverse(tree[p][2]) # note the use of the double indexing and the function recursively calling itself
